...
export default function KittySchemesPage() {
  ...
  const { data, isLoading } = trpc.india.schemes.kitty.list.useQuery({ page, limit: 20 } as never);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Kitty Schemes" description="Group savings schemes." breadcrumbs={[
      ...
      <div className="rounded-lg border">
        ...
        {isLoading ? (
          ...
          <EmptyState icon={<PiggyBank className="h-8 w-8" />} title="No kitty schemes" />
        ) : (
          <div className="divide-y">
            {items.map((s) => (
              <Link key={s.id as string} href={`/finance/schemes/kitty/${s.id}`} className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-4 px-4 py-3 text-sm hover:bg-accent">
